...
# train_octa_v3.py
#
# OCTA modalitesi için V3 eğitim script'i.
# V2'deki tüm teknikler korunarak genişletilmiş veri seti (525 görüntü) ile
# eğitim yapılır.
#
# V2'den farklılıklar:
#   - Veri seti: 338 → 525 görüntü (187 harici OptoVue kontrol eklendi)
#   - Source-aware loglama: Cihaz bazlı performans takibi
#   - Early stopping: Sabit epoch yerine patience tabanlı duruş
#   - AdamW optimizer: Weight decay düzenlileştirme
#
# Korunan V2 teknikleri:
#   - Agresif Data Augmentation (OCTADatasetV2)
#   - Progressive Fine-Tuning (5 epoch head → full)
#   - Label Smoothing (0.1)
#   - Cosine Annealing LR
#
# Çıktılar:
#   - outputs/models/octa_v3_resnet18_best.pth
#   - outputs/metrics/octa_v3_train_history.json
# ==============================================================================
...
EPOCHS = 40                # V2'de 30 idi, veri arttı → daha uzun eğitim
UNFREEZE_EPOCH = 5         # İlk 5 epoch sadece head eğitimi
BATCH_SIZE = 16
LABEL_SMOOTHING = 0.1      # 1 -> 0.95, 0 -> 0.05
PATIENCE = 10              # Early stopping: 10 epoch boyunca iyileşme yoksa dur
...
def print_data_summary(dataset, split_name):
    ...
    # Kaynak dağılımı (BH=Heidelberg, UT=OptoVue)
    def get_source(filepath):
        if "UT-OCTA" in str(filepath):
            return "OptoVue (UT)"
        else:
            return "Heidelberg (BH)"

    df_copy = df.copy()
    df_copy["device_source"] = df_copy["filepath"].apply(get_source)
    source_counts = df_copy["device_source"].value_counts().to_dict()
...
def main():
    print("=" * 60)
    print("OCTA V3 EĞİTİMİ — Genişletilmiş Veri Seti (525 görüntü)")
    print("=" * 60)
    ...
    # 3. Modeli Oluştur (PyTorch Pretrained ResNet-18)
    ...
    # Faz 1: Backbone dondur, sadece FC (Classifier) eğit
    print(f"\n{'─'*60}")
    ...
    patience_counter = 0
    history = []
    best_model_path = str(models_dir / "octa_v3_resnet18_best.pth")
    ...
    # Eğitim sonu: Tarihçeyi JSON olarak kaydet
